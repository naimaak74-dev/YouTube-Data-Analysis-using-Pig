...
files = FILTER all_data BY category IS NOT NULL AND views IS NOT NULL;

grouped_categories = GROUP files BY category;

top10_viewed_categories = FOREACH grouped_categories {
    sorted = ORDER files BY views DESC;
    top10 = LIMIT sorted 10;
    GENERATE FLATTEN(top10);
};

result = FOREACH top10_viewed_categories
    GENERATE videoid, category, views;

STORE result INTO '/youtubepig/output/Top10ViewedByCategories'
USING PigStorage('|');
